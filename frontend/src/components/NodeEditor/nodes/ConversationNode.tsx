  ...
  return (
    <div
      className={`rounded-lg border bg-surface px-3 py-2.5 shadow-sm transition-shadow ${
        selected ? "border-[#6b9fd4] shadow-md" : "border-line"
      }`}
      style={{ minWidth: 140 }}
    >
      <Handle type="target" position={Position.Left} className="!h-3 !w-3 !border-2 !border-base !bg-[#6b9fd4]" />
      <div className="flex items-center gap-2">
        <span className="h-2 w-2 rounded-full bg-[#6b9fd4]" />
      ...
      <Handle type="source" position={Position.Right} className="!h-3 !w-3 !border-2 !border-base !bg-[#6b9fd4]" />
    </div>
  );
}
